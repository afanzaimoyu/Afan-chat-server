...
class MessageInfo(Schema):
    ...
    sendTime: int = Field(..., description="消息发送时间")
    type: int = Field(..., description="消息类型 1正常文本 2.撤回消息")
    body: Any = Field(..., description="消息内容不同的消息类型")
...
class ChatMessageRespSchema(Schema):
    ...
    @classmethod
    def get_resp(cls, body, message):

        mark = message.messagemark_set.filter(type=MarkModel.Type.MARK).count()
        un_mark = message.messagemark_set.filter(type=MarkModel.Type.UN_MARK).count()
        user_mark = message.messagemark_set.filter(type=MarkModel.Type.MARK, user_id=message.from_user_id).exists()
        user_un_mark = message.messagemark_set.filter(type=MarkModel.Type.UN_MARK, user_id=message.from_user_id).exists()
        messageMark = MessageMark(likeCount=mark, dislikeCount=un_mark, userLike=user_mark,userDislike=user_un_mark)

        return cls(fromUser=UserInfo(uid=message.from_user_id),
                   message=MessageInfo(
                       id=message.id,
