...
class MessageMark(models.Model):
    ...
    status = models.IntegerField(choices=Status.choices, verbose_name="动作类型 1确认 2取消")
    create_time = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    update_time = models.DateTimeField(auto_now=True, verbose_name='修改时间')
    ...
    def mark_count(self):
        mark = MessageMark.objects.filter(msg=self.msg, type=self.Type.MARK).count()
        un_mark = MessageMark.objects.filter(msg=self.msg, type=self.Type.UN_MARK).count()
        return mark - un_mark
